...
- **内側スクロール領域（ChatGPT など）にも対応**。ページ本体がスクロールしない SPA では、最も大きくスクロールできる領域を探してその要素を `scrollTop` で動かし、撮影画像からはその領域だけを切り出して結合する。領域の DOM の外にあって領域に重なる要素（入力欄・「最下部へ」ボタン等）は「スクロールしても動かないオーバーレイ」として1コマ目から隠す。隠さないと、その下にある本文が毎コマ覆われて欠ける。
